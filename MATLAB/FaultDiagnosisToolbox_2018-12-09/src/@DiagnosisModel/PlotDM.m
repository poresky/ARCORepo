...
%               decomposition of M+
%
%    axislabels If true, puts equation and variable names on the axis. 
%               If model is large and/or long variable names, it could
%               be useful to set this to false. (default: true)
%
...
%  http://opensource.org/licenses/MIT)
  ...
  pa.addOptional('axislabels', true);
  pa.addOptional('fontsize', false);  
  pa.parse(varargin{:});
  opts = pa.Results;
    ...
    for kk=1:length(fPlotRowIdx)
      if ~any(fPlotRowIdx(kk)==fPlotRowIdx(1:kk-1)) % Has not yet been printed
        ...
        if verLessThan('matlab', '8.4')
          text( nVars+1.5, fPlotRowIdx(kk), faultLabel, 'Color', 'red', 'fontsize', fontSize);
        else
          text( nVars+1.5, fPlotRowIdx(kk), faultLabel, 'Color', 'red', 'interpreter', 'none', 'fontsize', fontSize );
        end
      end
    end    
  end
